...
function loadFromLocalStorage() {
    ...
    if (lists.length === 0) {
        ...
        addProduct('Palha de aco', 'Limpeza', 1, 'Unidade');
        ...
        .map(product =>
            `<div class="category-item ${product.completed ? 'completed' : ''}" onclick="toggleProductStatus(${product.id}, event)">
                ...
                <div class="menu-container">
                    <button class="menu-button" onclick="toggleMenu(${product.id}); event.stopPropagation();">
                        <i class="fas fa-ellipsis-v"></i>
                    </button>
                    <div id="menu-${product.id}" class="menu-options" style="display: none;">
                        <button onclick="openEditProductModal(${product.id}); event.stopPropagation();">Editar</button>
                        <button onclick="openDeleteModal(${product.id}, 'product', '${product.name}'); event.stopPropagation();">Excluir</button>
                    </div>
                </div>
            </div>`
        ).join('');
